...
toggleBtns.forEach(function(btn) {
  btn.addEventListener('click', function(event) {
    ...
    while (nextSibling) {
      if (nextSibling.tagName === 'DIV' && !nextSibling.classList.contains('toggle-child')) {
...
//   let childBlock = target.closest('.toggle-child');
//   if (!toggleBtn && !childBlock) {
//     let childBlocks = document.querySelectorAll('.toggle-child');
//     for (let i = 0; i < childBlocks.length; i++) {
//       childBlocks[i].style.display = 'none';
//     }
//   }
